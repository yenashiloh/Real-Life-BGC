 ...
 <div class="tab-pane fade profile-edit pt-3" id="profile-edit">
    ...
    {{-- PERSONAL INFORMATION --}}
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-6">
                <label for="incoming_grade" class="col-form-label" style="font-weight: normal;">Incoming Grade</label>
                <select name="incoming_grade" class="form-select" id="incoming_grade" disabled>
                    ...
                    @foreach(['Grade 7', 'Grade 8', 'Grade 9', 'Grade 10', 'Grade 11', 'Grade 12', 'First Year College', 'Second Year College', 'Third Year College', 'Fourth Year College'] as $grade)
                        <option value="{{ $grade }}" {{ isset($academicInfoData) && $academicInfoData->incoming_grade_year === $grade ? 'selected' : '' }}>{{ $grade }}</option>
                    @endforeach
                </select>
            </div>
            
            <div class="col-md-6"  >
                <label for="current_school" class="col-form-label" style="font-weight: normal;">Current School</label>
                <input name="current_school" type="text" class="form-control" id="current_school" value="{{ $academicInfoData->current_school}}">
            </div>

            @if (!empty($academicInfoData->current_course_program_grade))
                <div class="col-md-6" style="margin-top: 10px;">
                    <label for="current_course" class="col-form-label" style="font-weight: normal;">Current Course or Program</label>
                    <input name="current_course_program_grade" type="text" class="form-control" id="current_course" value="{{ $academicInfoData->current_course_program_grade }}">
                </div>
            @endif
          ...
          {{-- SCHOOL APPLICATION AND COURSE CHOICE --}}
            ...
            <div class="row mb-3">
                ...
                @if(!empty($academicInfoChoiceData->first_choice_school))
                    <div class="col-md-4">
                        <label for="first_choice_school" class="col-form-label" style="font-weight: normal;">First Choice School</label>
                        <input name="first_choice_school" type="text" class="form-control" id="first_choice_school" value="{{$academicInfoChoiceData->first_choice_school}}">
                    </div>
                @endif
                @if(!empty($academicInfoChoiceData->second_choice_school))
                    <div class="col-md-4">
                        <label for="second_choice_school" class="col-form-label" style="font-weight: normal;">Second Choice School</label>
                        <input name="second_choice_school" type="text" class="form-control" id="second_choice_school" value="{{$academicInfoChoiceData->second_choice_school}}">
                    </div>
                @endif
                @if(!empty($academicInfoChoiceData->third_choice_school))
                    <div class="col-md-4">
                        <label for="third_choice_school" class="col-form-label" style="font-weight: normal;">Third Choice School</label>
                        <input name="third_choice_school" type="text" class="form-control" id="third_choice_school" value="{{$academicInfoChoiceData->third_choice_school}}">
                    </div>
                @endif
            </div>
        @endif
        ...
        @if(!empty($academicInfoChoiceData->first_choice_course) || !empty($academicInfoChoiceData->second_choice_course) || !empty($academicInfoChoiceData->third_choice_course))
            ...
            <div class="row mb-3">
                ...
                @if(!empty($academicInfoChoiceData->first_choice_course))
                    <div class="col-md-4">
                        <label for="first_choice_course" class="col-form-label" style="font-weight: normal;">First Choice Course</label>
                        <input name="first_choice_course" type="text" class="form-control" id="first_choice_course" value="{{$academicInfoChoiceData->first_choice_course}}">
                    </div>
                @endif
                @if(!empty($academicInfoChoiceData->second_choice_course))
                    <div class="col-md-4">
                        <label for="second_choice_course" class="col-form-label" style="font-weight: normal;">Second Choice Course</label>
                        <input name="second_choice_course" type="text" class="form-control" id="second_choice_course" value="{{$academicInfoChoiceData->second_choice_course}}">
                    </div>
                @endif
                @if(!empty($academicInfoChoiceData->third_choice_course))
                    <div class="col-md-4">
                        <label for="third_choice_course" class="col-form-label" style="font-weight: normal;">Third Choice Course</label>
                        <input name="third_choice_course" type="text" class="form-control" id="third_choice_course" value="{{$academicInfoChoiceData->third_choice_course}}">
                    </div>
                @endif
            </div>
        @endif
        ...
        @foreach ($gwaFields as $fieldName => $label)
            @if (!empty($academicInfoGradesData->$fieldName))
                <div class="col-md-4 mb-3">
                    <label for="{{ $fieldName }}" class="col-form-label" style="font-weight: normal;">{{ $label }}</label>
                    <input name="{{ $fieldName }}" type="number" class="form-control gwa-input" id="{{ $fieldName }}" value="{{ $academicInfoGradesData->$fieldName }}" style="margin-bottom: 10px;">
                </div>
            @endif
        @endforeach
    </div>
            ...
            <div class="text-center">
                <button type="submit" class="btn btn-primary" style="margin-top: 13px;" id="save-changes-btn" {{ auth()->user()->status !== 'New Applicant' || in_array(auth()->user()->status, ['Under Review', 'Shortlisted', 'For Interview', 'For House Visitation', 'Declined']) ? 'disabled' : '' }}>Save Changes</button>
            </div>
 ...
 <script>
   ...
   $(document).ready(function () {
    $('#profile-form').submit(function (event) {
        ...
        $('input[type="text"], input[type="number"]').each(function () {
            ...
            if (emptyFields.length > 0) {
                ...
                var errorMessage = (emptyFields.length === 1 && emptyFields.includes('birthdate')) ? 'Birthdate should not be empty.' : 'The field(s) should not be empty.';
                var alertElement = $('<div class="alert alert-danger alert-dismissible fade show d-flex align-items-center justify-content-center" role="alert" style="width: 30%; margin: 0 auto;"></div>');
                ...
                if (age >= 26) {
                    ...
                    var ageAlert = $('<div class="alert alert-danger alert-dismissible fade show d-flex align-items-center justify-content-center" role="alert" style="width: 50%; margin: 0 auto;"></div>');
    ...
    form.addEventListener("submit", function(event) {
      ...
      if (error) {
        event.preventDefault();
        const errorMessage = "Applicants must have a General Weighted Average within the range of 88 to 100.";
