                  ...
                  @foreach($announcement->where('published', true)->sortByDesc('created_at') as $announce)
                      <div class="card mb-4" id="card-{{ $announce->id }}">
                          ...
                          <div class="card-body" id="card-body-{{ $announce->id }}">
                              @php
                                  $fullContent = preg_replace('/<img(.*?)src="data:image\/jpeg;base64,(.*?)"(.*?)>/i', '<a href="data:image/jpeg;base64,$2" target="_blank"$1$3><img$1src="data:image/jpeg;base64,$2"$3></a>', $announce->caption);
                                  $shortContent = substr(strip_tags($fullContent), 0, 500);
                              @endphp
                              <div id="short-content-{{ $announce->id }}">
                                  {!! $shortContent !!}
                                  @if(strlen(strip_tags($fullContent)) > 500)
                                      <span>...</span>
                                      <button class="btn btn-link p-0 mt-2" id="see-more-{{ $announce->id }}" onclick="toggleContent('{{ $announce->id }}')">See More</button>
                                  @endif
                              </div>
                              <div id="full-content-{{ $announce->id }}" style="display: none;">
                                  {!! $fullContent !!}
                                  <button class="btn btn-link p-0 mt-2" id="see-less-{{ $announce->id }}" onclick="toggleContent('{{ $announce->id }}')">See Less</button>
                              </div>
                          </div>
                      </div>
                  @endforeach
